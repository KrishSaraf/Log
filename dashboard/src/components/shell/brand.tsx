...
/**
 * Monogram plus wordmark. The lime tile is the only saturated element in the
 * chrome, which is what makes the accent read as the app's signature.
 */
export function Brand({ className }: { className?: string }) {
  return (
    <span className={cn("flex items-center gap-2.5", className)}>
      <span className="flex size-6 shrink-0 items-center justify-center rounded-md bg-lime text-[0.6875rem] font-semibold text-on-lime">
        K
      </span>
